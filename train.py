...
visual_degree = 50 #  hyperparamter for visualization: heatmap_max = hessian_max / visual_degree
visual_degree_block_hessian = [10, 100]
loss_type = 'CE'
optimizer_type = 'adam'
cmap = 'cividis'
gaussian_data = True#False
comment = 'gaussian-n_total-'+str(n_total)+'-'+model_type+'-dim-'+str(input_dim)+'-width-'+str(width)+'-class-'+str(n_classes)+ '-sample_per_class-'+str(n_samples_per_class)+'-adam-'+loss_type+'visiondegree'+str(visual_degree)+'T'+str(num_epochs)+'optimizer-'+optimizer_type +'lr-'+str(lr)
...
def generate_data(n_samples_per_class, n_classes, input_dim):
    ...
    for i in range(n_classes):
        ...
        if input_dim == 2:
            center = np.array([np.cos(2 * np.pi * i / n_classes), np.sin(2 * np.pi * i / n_classes)]) * 5  # Class centers on a circle
        else:
            center = np.random.rand(input_dim) * 10  # Random class center
        class_samples = np.random.randn(n_samples_per_class, input_dim) * 0.5 + center  # Add some noise
        X.append(class_samples)

        # assign random label
        y.extend([np.random.randint(0, n_classes)]*n_samples_per_class)
...
# generate Gaussian data, single sample
def generate_gaussian_data_single_sample(n_total, n_classes, input_dim):
    # Generate synthetic data for specified dimensions
    X = []
    y = []
...
if gaussian_data:
    #X, y = generate_gaussian_data(n_samples_per_class, n_classes, input_dim)
    X, y = generate_gaussian_data_single_sample(n_total, n_classes, input_dim)
else:
    X, y = generate_data(n_samples_per_class, n_classes, input_dim)
...
for epoch in range(num_epochs):
    ...
    avg_loss = loss.item() / (n_samples_per_class * n_classes)
...
def plot_results(): # might not use this
    with torch.no_grad():
        model.eval()
    ...
    plt.savefig(f'figure/dataset.png')
    ...
    plt.savefig(f'figure/{comment}_loss_T_{num_epochs}.png')
...
print(f"frobenious norm of full hessian = {np.linalg.norm(full_hessian, 'fro' )}")
